...
export async function getSchoolOverviewStats(schoolId: string): Promise<SchoolOverviewStats> {
  ...
  const [{ count: teachers }, { data: classes }, { count: resources }] = await Promise.all([
    supabase.from("school_members").select("id", { count: "exact", head: true }).eq("school_id", schoolId).eq("role", "teacher").eq("status", "approved"),
    supabase.from("classes").select("id").eq("school_id", schoolId),
    supabase.from("resources").select("id", { count: "exact", head: true }).eq("school_id", schoolId),
  ]);
  ...
  if (classIds.length > 0) {
    const [{ count: assignmentCount }, { data: roster }] = await Promise.all([
      ...
      supabase.from("class_members").select("student_id").in("class_id", classIds),
    ]);
  ...
  const studentIdsForPrefs = classIds.length
    ? (await supabase.from("class_members").select("student_id").in("class_id", classIds)).data ?? []
    : [];
  ...
  const { count: accessibilityUsage } = uniqueStudentIds.length
    ? await supabase.from("reading_preferences").select("user_id", { count: "exact", head: true }).in("user_id", uniqueStudentIds)
    : { count: 0 };
...
export async function getSchoolClasses(schoolId: string): Promise<SchoolClass[]> {
  ...
  const { data } = await supabase
    .from("classes")
    .select("id, name, grade, subject, profiles(first_name, last_name), class_members(count)")
    .eq("school_id", schoolId);

  return (data ?? []).map((c) => {
    const teacher = c.profiles as unknown as { first_name: string; last_name: string } | null;
    return {
      id: c.id,
      name: c.name,
      grade: c.grade,
...
export async function getSchoolResources(schoolId: string): Promise<SchoolResource[]> {
  ...
  const { data } = await supabase
    .from("resources")
    .select("id, title, subject, status, profiles(first_name, last_name)")
    .eq("school_id", schoolId)
    .order("created_at", { ascending: false });

  return (data ?? []).map((r) => {
    const owner = r.profiles as unknown as { first_name: string; last_name: string } | null;
