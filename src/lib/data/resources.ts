...
export async function getResource(resourceId: string) {
  ...
  const { data } = await supabase.from("resources").select("*").eq("id", resourceId).maybeSingle();
  return data;
}

export function resourceSections(resource: Tables<"resources">): ExtractedContent["sections"] {
