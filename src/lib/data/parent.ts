...
export async function getLinkedChildren(parentId: string): Promise<LinkedChild[]> {
  ...
  const { data } = await supabase
    ...
    .select("student_id, status, profiles(first_name, last_name)")
    .eq("parent_id", parentId);

  return (data ?? []).map((l) => {
    const profile = l.profiles as unknown as { first_name: string; last_name: string } | null;
    return {
...
export async function getChildOverview(studentId: string): Promise<ChildOverview> {
  ...
  const { data: profile } = await supabase.from("profiles").select("first_name, last_name").eq("id", studentId).maybeSingle();

  const { data: activity } = await supabase
    .from("progress_events")
    .select("event_type, created_at, resources(title)")
    .eq("student_id", studentId)
  ...
  const recentActivity = (activity ?? []).map((a) => ({
    ...
    resourceTitle: (a.resources as unknown as { title: string } | null)?.title ?? null,
  }));
  ...
  return {
    ...
    upcomingAssignments: (assignments ?? []).map((a) => ({
      ...
      title: a.title,
      dueDate: a.due_date,
      className: (a.classes as unknown as { name: string } | null)?.name ?? "Class",
    })),
  };
}
...
export async function getChildrenResources(parentId: string): Promise<ParentResourceItem[]> {
  ...
  for (const link of links ?? []) {
    const profile = link.profiles as unknown as { first_name: string; last_name: string } | null;
    const childName = profile ? profile.first_name : "Child";

    const { data: memberships } = await supabase.from("class_members").select("class_id").eq("student_id", link.student_id);
    const classIds = (memberships ?? []).map((m) => m.class_id);
    if (classIds.length === 0) continue;

    const { data: assignments } = await supabase
      .from("assignments")
      .select("resources(id, title, subject)")
      .in("class_id", classIds)
      .not("resource_id", "is", null);

    for (const a of assignments ?? []) {
      const resource = a.resources as unknown as { id: string; title: string; subject: string | null } | null;
